...
with col_left:
    ...
    client_data = st.text_area("Paste Input:", height=400, placeholder="e.g., Male, 41, Sleeps 5 hours, High work stress...")
    
    if st.button("Generate Plan"):
        if client_data:
            with st.spinner("Analyzing Decision Rules..."):
                try:
                    client = OpenAI(base_url="https://integrate.api.nvidia.com/v1", api_key=api_key)
                    
                    # REASONING CHAIN PROMPT (The "Hassan Brain")
                    prompt = f"""
                    SYSTEM: You are the Thrive Hub Decision Engine. 
                    ...
                    3. If Constant Soreness/Flatness -> PIVOT to 'Recovery Debt'. Schedule deload.
                    ...
                    STRICT RULES: English only. Use Hassan's coaching lines. Pull specific meal data.
    ...
    if 'result' in st.session_state:
        ...
        if st.button("Save to Brain"):
            if append_to_brain(st.session_state.last_input, st.session_state.result):
                st.balloons()
                st.success("Logic Injected into the Thrive Hub Brain.")
